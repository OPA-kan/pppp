...
export function interpretPADC(scores: PADCScore): EmotionInterpretation {
  ...
  // 2. センチメント判定
  const sentiment = P > 0.5 ? "ポジティブ 🌟" : P > 0.35 ? "ニュートラル 😐" : "ネガティブ 😔";
  ...
  return {
    ...
    intensity,
    intensityValue,
    description: `${primaryEmotion}の状態です。${sentimentColor === "bg-green-100 text-green-800 border-green-300" ? "ポジティブな" : sentimentColor === "bg-blue-100 text-blue-800 border-blue-300" ? "ニュートラルな" : "ネガティブな"}感情が${intensity}強さで表現されています。`,
    details: {
      valence: valenceDesc,
      arousal: arousalDesc,
...
function getMixedEmotions(P: number, A: number, D: number, C: number): string[] {
  ...
  // P と A の組み合わせ
  if (P > 0.6 && A > 0.6 && D > 0.6) {
    mixed.push("達成感 (Achievement)");
    mixed.push("優越感 (Superiority)");
  }
  if (P < 0.4 && A > 0.6 && D > 0.6) {
    mixed.push("激怒 (Rage)");
    mixed.push("執念 (Obsession)");
  }
  if (P < 0.4 && A > 0.7 && D < 0.4) {
    mixed.push("パニック (Panic)");
    mixed.push("絶望 (Despair)");
  }
  if (P > 0.6 && A < 0.4 && D > 0.5) {
    mixed.push("満足感 (Fulfillment)");
    mixed.push("充実感 (Fulfillment)");
  }
  if (P > 0.5 && A > 0.5 && C > 0.7) {
    mixed.push("確信的喜び (Confident Joy)");
  }
  if (P < 0.4 && A < 0.4 && C < 0.3) {
